...
public readonly partial record struct Result<TValue>
{
    ...
    {
        if (IsFailure) return Result<TOut>.Failure(Errors).AsTask();
        return func(Value);
    }
    ...
    {
        if (IsFailure) return Result.Failure(Errors).AsTask();
        return func(Value);
    }
    ...
    {
        if (IsFailure) return Result.Failure<TOut>(Errors).AsValueTask();
    ...
    {
        if (IsFailure) return Result.Failure(Errors).AsValueTask();
        return valueTask(Value);
    }
}
