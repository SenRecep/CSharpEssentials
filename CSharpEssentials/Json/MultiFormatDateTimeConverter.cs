...
{
    private readonly static Type _dateTimeType = typeof(DateTime);
    private readonly static Type _nullableDateTimeType = typeof(DateTime?);
    private readonly static Type _convertType = typeof(MultiFormatDateTimeConverter<>);
...
public sealed class MultiFormatDateTimeConverter<T> : JsonConverter<T>
{
    ...
    public override void Write(Utf8JsonWriter writer, T value, JsonSerializerOptions options)
    {
        ...
        {
            writer.WriteStringValue(dateTime.Value.ToString(_defaultFormat));
        }
        else if (_isNullable)
        {
